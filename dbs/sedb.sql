...
-- Dumping structure for table sanienterprisedb.patron
...
CREATE TABLE IF NOT EXISTS `patron` (
  ...
  PRIMARY KEY (`user_id`),
  UNIQUE KEY `UK_amqkum6j4mbmki52u7o72rr24` (`email`),
  UNIQUE KEY `UK_pft0f8bs7isbc9oq5dras7s` (`id_number`),
  UNIQUE KEY `UK_sirf1w21fn8a8urjlsej9nydr` (`admin_username`),
  UNIQUE KEY `UK_q0mncvgwak0cmpp9so549wmwe` (`fk_account_id`),
  UNIQUE KEY `UK_8l1wfcuyb2fwhaplvkdbipw25` (`fk_address_id`),
  CONSTRAINT `FK8m857e9f5x62noe7gpxpomd00` FOREIGN KEY (`fk_account_id`) REFERENCES `account` (`account_id`),
  CONSTRAINT `FKks7ldjch9hlkrb9q10t7m9vab` FOREIGN KEY (`fk_address_id`) REFERENCES `address` (`address_id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `product` (
  ...
  `category` enum('TV_STAND','COFFEE_TABLE','CONSOLE_TABLE','OCCASIONAL_CHAIR','CARPET','DINING_SET','BAR_CHAIR','LOVE_SEAT','HEADBOARD','PEDESSTAL','DRESSING_TABLE','SLEEPER_COUCH','STUDY_TABLE','CHEST_OF_DRAWER','USED_FRIDGE','BATHROOM_CADDY') NOT NULL,
